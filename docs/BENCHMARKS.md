# Krill vs Ollama — Benchmarks

Reproducible head-to-head benchmarks across the four axes Krill serves
...
sweep (the axis where Krill's continuous batcher wins architecturally).
...
| Krill | commit `c0b4a63` (release build, `swift build -c release`) |
| Ollama | 0.30.6 (GGUF default; MLX engine available for Gemma 4) |
| Common model | Gemma 4 E2B — Krill `gemma-4-e2b` / Ollama `gemma4:e2b` (GGUF) (the one model that does text+vision+voice+tools) |
...
These rows compare Krill (MLX) against Ollama's **GGUF / llama.cpp** path (the
...
| Model | State | Metric | Krill | Ollama | Krill |
|---|---|---|--:|--:|:--|
...
is the real single-stream headroom, not "by miles."** Krill's clearer wins are
...
Krill's single-stream decode runs ~15% behind Ollama (0.85x) and the concurrent
...
the 14B tracks the small/mid models. Krill leads on every model that fits
comfortably.
...
gone. Krill uses `mlx-community/gemma-4-e2b-it-4bit` (affine 4-bit) — not a
bit-identical quant to nvfp4, but both are MLX 4-bit.

| Metric | Krill (MLX affine-4bit) | Ollama `gemma4:e2b-mlx` (MLX nvfp4) | Krill |
|---|--:|--:|:--|
...
| vision / voice | ✅ native | ❌ MLX tag is text-only | Krill only |

> This is a SEPARATE, later measurement run from the GGUF tables above (different
> session, thermal state, and build), so Krill's absolute self-numbers here
...
**Read (honest):** on **single-stream** text decode, Krill is at **parity with
...
single-stream MLX win. Krill's durable wins over Ollama's MLX engine are
...
tag is text-only; Krill serves vision + voice natively). Numbers drift ~±5% with
...
Ollama ingests the audio but emits no text. **Krill transcribes correctly.**
NOTE: Krill voice currently works via the native CLI (`krill run --audio`),
**not** the HTTP API — see `docs/BENCHMARK_ISSUES.md` #1.

### Tool calling (single-shot, scored)

| Model | Metric | Krill | Ollama |
|---|---|--:|--:|
...
and Krill's edge is **latency per call** (it inherits the decode/total wins). On
**Gemma 4 E2B** Krill emits valid tool calls **4/4** where Ollama's `gemma4:e2b`
returns **0/4** — a same-model correctness win (Krill's native per-family tool
...
| N streams | Krill agg tok/s | Ollama agg tok/s | Ratio |
|--:|--:|--:|--:|
| 1 | 97.8 | 67.9 | 1.44x |
| 2 | 120.0 | 74.0 | 1.62x |
| 4 | 117.1 | 75.7 | 1.55x |
...
**This is the architectural win:** Krill aggregate throughput *climbs* with load
(98 → 155 tok/s) because the continuous batcher serves many decode rows from one
weight read; Ollama is flat (~68-77) because it serializes. At N=8 Krill is
...
schemas + retrieved docs) across many calls with a short varying tail. Krill now
...
|--:|--:|--:|
| **Krill** | 4556 ms | **180 ms** |
| **Ollama**  | 4781 ms | 193 ms |

Krill's repeated-context prefill is **180 ms, at parity with Ollama's 193 ms** -
...
cold within bf16 noise + first-token match - see `docs/BACKLOG.md`). Krill
...
| Model | N | Krill tok/s | Ollama tok/s | Krill tasks/s | Ollama tasks/s |
|---|--:|--:|--:|--:|--:|
...
On **Gemma 4 E2B** Krill runs the agentic/RAG workload at **~2x** Ollama's
...
varied context). On **Llama 3.2 3B** Krill leads **1.2–1.4x**. JSON validity is
8/8 on both engines.

**14B agentic is RAM-bound on this box.** On `qwen2.5-14b` the concurrent agentic
path does NOT lead (Krill ~4.7 vs Ollama ~11 tok/s, and N=4 slower than N=1) —
...
| ctx tokens | Krill s | Ollama s | Krill |
|--:|--:|--:|:--|
| ~16k | **90.8** | 120.3 | **1.3x faster** |
| ~53k | **345** | 644-686 | **1.9-2.0x faster** |
| ~99k | **746** | 2701-3057 | **3.6-4.1x faster** |

**Memory at ~99k:** Krill peaks ~18.5GB inside the box's budget (RotatingKVCache
...
(the ranges above), Krill's does not. Needle retrieval was correct in every cell
on both engines. Krill's usable ceiling is ~123k at 15.6 tok/s
...
Krill hits the shared-prefix KV cache - prefill drops 88.5s -> **0.1s**. Ollama
...
post-prefill overhead (on Krill's side the O(ctx) prefix-cache store), NOT the
...
Krill's mixed-weights blob, not `gemma-4-12b-nvfp4`). Re-measured on the nvfp4
...
near 21k) and Krill prefill leads at every rung (161 vs 178 s at ~50k). But the
...
| ctx | Krill tok/s (engine / serve) | Ollama GGUF tok/s | verdict |
|--:|--|--|--|
| ~2k | 81.7 | (short-ctx table: Krill 1.08x) | Krill |
...
mostly past the model's useful range; (2) early Krill readings of a "cliff" at
...
```bash
# 1. Build the release binary (needed for the Krill cold/CLI path).
make release

# 2. Bring up both engines (Ollama on 11434 already; Krill on its default 57455).
#    For the concurrency sweep, enable the batcher + n-gram spec.
KRILL_NUM_PARALLEL=16 KRILL_NGRAM_SPEC=1 \
  .build/release/krill serve --model gemma-4-e2b &
...
python3 tools/krill_concurrent_benchmark.py \
  --krill-url http://127.0.0.1:57455 --krill-model gemma-4-e2b \
...
## Companion harnesses
- `tools/krill_concurrent_benchmark.py` — averaged N-stream sweep (serial vs batched arms, p99 TTFT).
