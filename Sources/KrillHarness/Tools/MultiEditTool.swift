...
public struct MultiEditTool: Tool {
    ...
    public func run(argumentsJSON: String) async -> ToolResult {
        ...
        let originalText = String(decoding: data, as: UTF8.self)
        ...
        let hunks = FileToolSupport.unifiedDiff(old: originalText, new: text)
        let stat = FileToolSupport.diffstat(hunks: hunks)
        let preview = FileToolSupport.compactPreview(hunks: hunks)
        return ToolResult(
            content: "Applied \(edits.count) edit(s) to \(displayPath) (\(stat), \(total) replacement(s))."
                + (preview.isEmpty ? "" : "\n" + preview),
            isError: false,
