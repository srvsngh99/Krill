...
public struct ModelAdapter: Sendable, Equatable {
    ...
    public var chatTemplate: ChatTemplatePolicy {
        ...
        case .phi:
            ...
            return .phi
        case .nanbeige:
            // Not a fallback: Nanbeige's own chat template emits tool calls
            // wrapped in `<tool_call>`, and its `tool_call_format='json'` mode is
            // literally `<tool_call>{"name": …, "arguments": {…}}</tool_call>` -
            // the exact string `toolSystemPrompt` asks for and `.hermes` parses.
...
///
/// This is a stable, module-neutral identifier. KrillServer's
