...
///
/// Skipped unless `KRILL_NGRAM_MODEL_PATH` points at a plain-causal checkpoint
...
final class NgramLiveParityTests: XCTestCase {
    ...
    private func requireModelDirectory() throws -> URL {
        guard let path = ProcessInfo.processInfo.environment["KRILL_NGRAM_MODEL_PATH"], !path.isEmpty else {
            throw XCTSkip("KRILL_NGRAM_MODEL_PATH not set")
        }
        var isDir: ObjCBool = false
        guard FileManager.default.fileExists(atPath: path, isDirectory: &isDir), isDir.boolValue else {
            throw XCTSkip("KRILL_NGRAM_MODEL_PATH is not a directory: \(path)")
        }
    ...
    func testNgramDecodeSpeedupObservation() async throws {
        guard ProcessInfo.processInfo.environment["KRILL_NGRAM_PERF"] != nil else {
            throw XCTSkip("Set KRILL_NGRAM_PERF to run the n-gram decode-speedup observation")
        }
        let dir = try requireModelDirectory()
        let engine = InferenceEngine(modelDirectory: dir)
        try await engine.load()
