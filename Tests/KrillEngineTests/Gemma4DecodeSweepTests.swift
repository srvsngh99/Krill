...
///
///   KRILL_RUN_DECODE_SWEEP=1 \
///   KRILL_GEMMA4_UNIFIED_MODEL_PATH=~/.krill/models/blobs/gemma-4-12b \
///     swift test --filter Gemma4DecodeSweepTests
///
/// Optional: KRILL_SWEEP_CTX="512,2048,6656" overrides the context list;
/// KRILL_SWEEP_NGRAM=0 skips the n-gram column (halves runtime).
final class Gemma4DecodeSweepTests: XCTestCase {

    private func requireOptIn() throws -> URL {
        guard ProcessInfo.processInfo.environment["KRILL_RUN_DECODE_SWEEP"] == "1" else {
            throw XCTSkip("KRILL_RUN_DECODE_SWEEP != 1")
        }
        guard let path = ProcessInfo.processInfo.environment["KRILL_GEMMA4_UNIFIED_MODEL_PATH"],
              !path.isEmpty else {
            throw XCTSkip("KRILL_GEMMA4_UNIFIED_MODEL_PATH not set")
        }
    ...
    private func prompt(sections: Int) -> String {
        ...
        // artifact, not a retrieval failure). KRILL_SWEEP_DIRECT_Q=1 asks for
        // the phrase ONLY - use it to validate retrieval at frontier lengths.
        if ProcessInfo.processInfo.environment["KRILL_SWEEP_DIRECT_Q"] == "1" {
    ...
    func testDecodeContextSweep() async throws {
        ...
        if let raw = env["KRILL_SWEEP_CTX"] {
        ...
        let runNgram = env["KRILL_SWEEP_NGRAM"] != "0"
