...
/// `KRILL_RERANKER_MODEL_PATH` because the test downloads ~2GB of weights
/// otherwise and we do not want CI doing that implicitly. Run locally
/// with:
///
///     KRILL_RERANKER_MODEL_PATH=$HOME/.krill/models/blobs/bge-reranker-v2-m3 \
///       swift test --filter RerankEngineTests
final class RerankEngineTests: XCTestCase {

    private func liveModelDir() throws -> URL {
        guard let path = ProcessInfo.processInfo.environment["KRILL_RERANKER_MODEL_PATH"],
              !path.isEmpty else {
            throw XCTSkip("KRILL_RERANKER_MODEL_PATH not set; skipping live reranker test")
        }
        let url = URL(fileURLWithPath: path)
        var isDir: ObjCBool = false
        guard FileManager.default.fileExists(atPath: url.path, isDirectory: &isDir),
              isDir.boolValue else {
            throw XCTSkip("KRILL_RERANKER_MODEL_PATH does not point to a directory: \(path)")
        }
        return url
    }
