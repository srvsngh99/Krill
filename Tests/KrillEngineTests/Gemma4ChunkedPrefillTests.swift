...
final class Gemma4ChunkedPrefillTests: XCTestCase {
    ...
    /// wall is ~21k.) Opt-in only so it never crashes CI: KRILL_RUN_OOM_BASELINE=1.
    func testSingleShotOOMsAtLongContext() async throws {
        guard ProcessInfo.processInfo.environment["KRILL_RUN_OOM_BASELINE"] == "1" else {
            throw XCTSkip("KRILL_RUN_OOM_BASELINE != 1 (this test intentionally crashes)")
        }
        let modelDir = try requireModelDirectory()
