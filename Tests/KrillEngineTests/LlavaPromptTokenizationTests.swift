...
/// `KRILL_LLAVA_MODEL_PATH` points at a llava-1.5 checkpoint directory (CI
...
final class LlavaPromptTokenizationTests: XCTestCase {
    ...
    private func requireTokenizer() async throws -> KrillTokenizer {
        guard let path = ProcessInfo.processInfo.environment["KRILL_LLAVA_MODEL_PATH"],
              !path.isEmpty else {
            throw XCTSkip("KRILL_LLAVA_MODEL_PATH not set")
        }
        var isDir: ObjCBool = false
        guard FileManager.default.fileExists(atPath: path, isDirectory: &isDir),
              isDir.boolValue else {
            throw XCTSkip("KRILL_LLAVA_MODEL_PATH is not a directory: \(path)")
        }
