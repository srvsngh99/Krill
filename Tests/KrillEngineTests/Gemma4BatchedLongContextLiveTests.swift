...
///
/// Set `KRILL_BATCH_MODEL_PATH` to a Gemma 4 checkpoint (e.g. gemma-4-e2b).
final class Gemma4BatchedLongContextLiveTests: XCTestCase {
    ...
    func testBatchedLongContextRetrievesNeedle() async throws {
        guard let p = ProcessInfo.processInfo.environment["KRILL_BATCH_MODEL_PATH"], !p.isEmpty
        else { throw XCTSkip("KRILL_BATCH_MODEL_PATH not set") }
