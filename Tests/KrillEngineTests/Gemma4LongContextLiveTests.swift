...
///
/// Set `KRILL_GEMMA4_MODEL_PATH` to a Gemma 4 checkpoint (e.g. gemma-4-e2b).
final class Gemma4LongContextLiveTests: XCTestCase {
    ...
    private func modelDir() throws -> URL {
        guard let path = ProcessInfo.processInfo.environment["KRILL_GEMMA4_MODEL_PATH"],
              !path.isEmpty else { throw XCTSkip("KRILL_GEMMA4_MODEL_PATH not set") }
        var isDir: ObjCBool = false
        guard FileManager.default.fileExists(atPath: path, isDirectory: &isDir), isDir.boolValue
        else { throw XCTSkip("KRILL_GEMMA4_MODEL_PATH is not a directory: \(path)") }
