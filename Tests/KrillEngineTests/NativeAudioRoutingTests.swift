...
/// always runs on the native Swift+MLX USM path. These KRILL_GEMMA4_MODEL_PATH
...
final class NativeAudioRoutingTests: XCTestCase {
    ...
    /// generation). Skips unless KRILL_GEMMA4_MODEL_PATH points at a real
    /// checkpoint.
    func testLiveNativeAudioProducesOutput() async throws {
        ...
        guard FileManager.default.fileExists(atPath: path, isDirectory: &isDir),
            ...
            throw XCTSkip("KRILL_GEMMA4_MODEL_PATH is not a directory")
        }
        let assets = ProcessInfo.processInfo.environment["KRILL_BENCH_ASSETS_DIR"]
    ...
    /// signal the benchmark measured. Skips unless KRILL_GEMMA4_MODEL_PATH points
