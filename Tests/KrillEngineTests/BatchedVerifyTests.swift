...
///
/// Gated on `KRILL_BATCH_MODEL_PATH` (a plain-causal checkpoint), matching the
...
final class BatchedVerifyTests: XCTestCase {
    ...
    func testBatchedNgramSpecThroughputObservation() async throws {
        guard ProcessInfo.processInfo.environment["KRILL_BATCH_SPEC_PERF"] != nil else {
            throw XCTSkip("Set KRILL_BATCH_SPEC_PERF to run the batched-spec throughput observation")
        }
        let dir = try requireModelDirectory()
        let engine = InferenceEngine(modelDirectory: dir)
        try await engine.load()
