...
///
/// Set `KRILL_TEXT_MODEL_PATH` to a standard per-layer-cache text checkpoint
/// (e.g. qwen2.5-3b). Gemma 4 has its own variant
/// (`Gemma4PartialReuseLiveTests`, env `KRILL_GEMMA4_MODEL_PATH`) because its
...
final class PrefixCachePartialReuseLiveTests: XCTestCase {
    ...
    private func modelDir() throws -> URL {
        guard let path = ProcessInfo.processInfo.environment["KRILL_TEXT_MODEL_PATH"],
              !path.isEmpty else {
            throw XCTSkip("KRILL_TEXT_MODEL_PATH not set")
        }
        var isDir: ObjCBool = false
        guard FileManager.default.fileExists(atPath: path, isDirectory: &isDir),
              isDir.boolValue else {
            throw XCTSkip("KRILL_TEXT_MODEL_PATH is not a directory: \(path)")
        }
