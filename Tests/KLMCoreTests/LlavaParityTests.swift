...
final class LlavaParityTests: XCTestCase {
    ...
    func testNativeLlavaPyTorchConvLayoutMatchesMLXVLMLogits() throws {
        guard let dirPath = ProcessInfo.processInfo.environment["KLM_LLAVA_PARITY_PT_DIR"] else {
            throw XCTSkip("Set KLM_LLAVA_PARITY_PT_DIR (see tools/verify_llava_parity.py <dir> pytorch)")
        }
        try runParity(dirPath)
    }
