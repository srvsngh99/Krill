...
import MLX
import MLXRandom
@testable import KrillKernels

/// Tests for the fused affine-4bit dequant + GEMV decode probe
/// (`KrillKernels.fusedQ4Gemv`). The numerical test is the correctness gate (the
/// kernel must match MLX's built-in `quantizedMatmul` bit-for-bit-ish); the
/// benchmark is gated on `KRILL_FUSED_Q4_BENCH` and only prints timings (it never
...
final class FusedQ4KernelTests: XCTestCase {
    ...
    func testBenchmarkVsBuiltin() throws {
        guard ProcessInfo.processInfo.environment["KRILL_FUSED_Q4_BENCH"] == "1" else {
            throw XCTSkip("Set KRILL_FUSED_Q4_BENCH=1 to run the fused-Q4 benchmark")
        }
        ...
        let fusedUs = time {
            KrillKernels.fusedQ4Gemv(x: x, w: w, scales: scales, biases: biases, groupSize: gs)
        }
