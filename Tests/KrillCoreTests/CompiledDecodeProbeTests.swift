...
///      data, not conjecture.
///
/// Gated on `KRILL_DECODE_PROBE` (it allocates real-size tensors and runs timing
...
final class CompiledDecodeProbeTests: XCTestCase {
    ...
    func testCompiledFixedBufferMatchesGrowingCache() throws {
        guard ProcessInfo.processInfo.environment["KRILL_DECODE_PROBE"] != nil else {
            throw XCTSkip("Set KRILL_DECODE_PROBE to run the compiled-decode probe")
        }
        MLXRandom.seed(0)
        let w = makeWeights()
        let capacity = 256
