...
///
/// Gated on `KRILL_MIXTRAL_PARITY_DIR`, a directory produced by
...
final class MixtralParityTests: XCTestCase {
    ...
    func testNativeMixtralMatchesMLXLMLogits() throws {
        guard let dirPath = ProcessInfo.processInfo.environment["KRILL_MIXTRAL_PARITY_DIR"] else {
            throw XCTSkip("Set KRILL_MIXTRAL_PARITY_DIR (see tools/verify_mixtral_parity.py)")
        }
