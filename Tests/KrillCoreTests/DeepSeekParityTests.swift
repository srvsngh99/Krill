...
/// mlx-lm. Gated on `KRILL_DEEPSEEK_V2_PARITY_DIR`, a directory produced by
...
/// unset.
///
/// DeepSeek-V3 is also covered, via `KRILL_DEEPSEEK_V3_PARITY_DIR`
...
final class DeepSeekParityTests: XCTestCase {
    ...
    func testNativeDeepSeekV2MatchesMLXLMLogits() throws {
        try runParity("KRILL_DEEPSEEK_V2_PARITY_DIR")
    }

    func testNativeDeepSeekV3MatchesMLXLMLogits() throws {
        try runParity("KRILL_DEEPSEEK_V3_PARITY_DIR")
    }
    ...
    func testNativeDeepSeekV3DecodeMatchesPrefill() throws {
        guard let dirPath = ProcessInfo.processInfo.environment["KRILL_DEEPSEEK_V3_PARITY_DIR"] else {
            throw XCTSkip("Set KRILL_DEEPSEEK_V3_PARITY_DIR (see tools/verify_deepseek_parity.py)")
        }
        let dir = URL(fileURLWithPath: dirPath)
        let refData = try Data(contentsOf: dir.appendingPathComponent("reference_logits.json"))
        let ref = try JSONDecoder().decode(Reference.self, from: refData)
