...
/// Gated on `KRILL_LLAVA_PARITY_DIR`, a directory produced by
...
final class LlavaParityTests: XCTestCase {
    ...
    func testNativeLlavaMatchesMLXVLMLogits() throws {
        guard let dirPath = ProcessInfo.processInfo.environment["KRILL_LLAVA_PARITY_DIR"] else {
            throw XCTSkip("Set KRILL_LLAVA_PARITY_DIR (see tools/verify_llava_parity.py)")
        }
        try runParity(dirPath)
    }

    /// Same parity, but the fixture ships the CLIP patch-embed Conv2d weight in
    /// PyTorch `[out, in, kH, kW]` layout (what a raw HF llava-1.5 checkpoint
    /// has), so this pins the loader's PyTorch->MLX transpose. Generated by
    /// `tools/verify_llava_parity.py <dir> pytorch`.
    func testNativeLlavaPyTorchConvLayoutMatchesMLXVLMLogits() throws {
        guard let dirPath = ProcessInfo.processInfo.environment["KRILL_LLAVA_PARITY_PT_DIR"] else {
            throw XCTSkip("Set KRILL_LLAVA_PARITY_PT_DIR (see tools/verify_llava_parity.py <dir> pytorch)")
        }
        try runParity(dirPath)
    }

    private func runParity(_ dirPath: String) throws {
        ...
        let pixels = try MLX.loadArrays(
            url: dir.appendingPathComponent("inputs/pixel_values.safetensors"))["pixel_values"]!
