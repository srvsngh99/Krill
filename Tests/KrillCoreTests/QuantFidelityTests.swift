...
///   KRILL_QF_BF16 = full-precision reference checkpoint
///   KRILL_QF_A    = quant A (e.g. naive uniform 4-bit)
///   KRILL_QF_B    = quant B (e.g. our mixed nvfp4)
final class QuantFidelityTests: XCTestCase {
    ...
    func testMixedNvfp4VsNaive4bitFidelity() throws {
        ...
        guard let bf16 = env["KRILL_QF_BF16"], let a = env["KRILL_QF_A"], let b = env["KRILL_QF_B"] else {
            throw XCTSkip("Set KRILL_QF_BF16, KRILL_QF_A, KRILL_QF_B (see docs)")
        }
        let labelA = env["KRILL_QF_A_LABEL"] ?? "A"
        let labelB = env["KRILL_QF_B_LABEL"] ?? "B"

        // Discover vocab from the reference, build the probe sequence. Prefer a
        // REAL tokenized paragraph passed via KRILL_QF_TOKENS (comma-separated ids)
        ...
        let toks: [Int32]
        if let real = env["KRILL_QF_TOKENS"] {
