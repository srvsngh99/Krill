...
///
/// Gated on `KRILL_OLMOE_PARITY_DIR`, a directory produced by
...
final class OLMoEParityTests: XCTestCase {
    ...
    func testNativeOLMoEMatchesMLXLMLogits() throws {
        guard let dirPath = ProcessInfo.processInfo.environment["KRILL_OLMOE_PARITY_DIR"] else {
            throw XCTSkip("Set KRILL_OLMOE_PARITY_DIR (see tools/verify_olmoe_parity.py)")
        }
