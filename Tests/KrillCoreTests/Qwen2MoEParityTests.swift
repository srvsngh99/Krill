...
/// Logit-parity check for the native Qwen 2 MoE runtime against mlx-lm.
///
/// Gated on `KRILL_QWEN2_MOE_PARITY_DIR`, a directory produced by
/// `tools/verify_qwen2_moe_parity.py` (tiny quantized Qwen2-MoE +
...
/// router (softmax-over-all, top-K, NO renorm), the sigmoid-gated shared
/// expert, and the `gatherQuantizedMM` SwitchGLU. Skipped when unset.
final class Qwen2MoEParityTests: XCTestCase {
    ...
    func testNativeQwen2MoEMatchesMLXLMLogits() throws {
        guard let dirPath = ProcessInfo.processInfo.environment["KRILL_QWEN2_MOE_PARITY_DIR"] else {
            throw XCTSkip("Set KRILL_QWEN2_MOE_PARITY_DIR (see tools/verify_qwen2_moe_parity.py)")
        }
