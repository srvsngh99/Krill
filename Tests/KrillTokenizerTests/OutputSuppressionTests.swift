...
/// tokenizer. The test loads it from `KRILL_GEMMA_TOKENIZER_DIR` or the default
...
final class OutputSuppressionTests: XCTestCase {
    private func gemmaTokenizerDirIfPresent() -> URL? {
        ...
        var candidates: [URL] = []
        if let env = ProcessInfo.processInfo.environment["KRILL_GEMMA_TOKENIZER_DIR"] {
    ...
    func testGemmaMediaMarkersAreSuppressedInOutputOnly() async throws {
        guard let dir = gemmaTokenizerDirIfPresent() else {
            throw XCTSkip("No Gemma-4 tokenizer present; set KRILL_GEMMA_TOKENIZER_DIR to run.")
        }
        let tok = try await KrillTokenizer(from: dir)
