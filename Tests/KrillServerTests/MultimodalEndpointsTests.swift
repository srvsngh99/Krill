...
/// ``Tests/KrillEngineTests/Gemma4SmokeTests.swift``: set
/// ``KRILL_GEMMA4_MODEL_PATH`` to enable; absent or invalid path leads to
/// ``XCTSkip``.
final class MultimodalEndpointsTests: XCTestCase {
    ...
    func testTwoDifferentImagesProduceDifferentOutputs() async throws {
        ...
        let path = ProcessInfo.processInfo.environment["KRILL_GEMMA4_MODEL_PATH"]!
    ...
    private func requireGemma4Env() throws {
        guard let path = ProcessInfo.processInfo.environment["KRILL_GEMMA4_MODEL_PATH"],
              !path.isEmpty else {
            throw XCTSkip("KRILL_GEMMA4_MODEL_PATH not set; skipping live multimodal endpoint test")
        }
        var isDir: ObjCBool = false
        guard FileManager.default.fileExists(atPath: path, isDirectory: &isDir),
              isDir.boolValue else {
            throw XCTSkip("KRILL_GEMMA4_MODEL_PATH does not point to a directory: \(path)")
        }
    }
