...
///   KLM_RUN_SDPA_PROBE=1 swift test --filter Gemma4PrefillSDPAProbeTests
final class Gemma4PrefillSDPAProbeTests: XCTestCase {

    private func requireOptIn() throws {
        guard ProcessInfo.processInfo.environment["KLM_RUN_SDPA_PROBE"] == "1" else {
            throw XCTSkip("KLM_RUN_SDPA_PROBE != 1")
        }
    }
    ...
    /// Run in isolation: KLM_RUN_SDPA_PROBE=1 swift test --filter testCausalSweepBoundedMemory
    ...
    /// .causal result: KLM_RUN_SDPA_PROBE=1 swift test --filter testArraySweepHitsOOM
