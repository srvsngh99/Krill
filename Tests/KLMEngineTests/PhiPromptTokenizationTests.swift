...
///
/// Runs only when `KLM_PHI_MODEL_PATH` points at a phi-4-mini checkpoint
...
final class PhiPromptTokenizationTests: XCTestCase {
    ...
    private func requireTokenizer() async throws -> KLMTokenizer {
        guard let path = ProcessInfo.processInfo.environment["KLM_PHI_MODEL_PATH"],
              !path.isEmpty else {
            throw XCTSkip("KLM_PHI_MODEL_PATH not set")
        }
        var isDir: ObjCBool = false
        guard FileManager.default.fileExists(atPath: path, isDirectory: &isDir),
              isDir.boolValue else {
            throw XCTSkip("KLM_PHI_MODEL_PATH is not a directory: \(path)")
        }
